...
// Helper para obtener estado de factura
const getEstadoFactura = (factura: FacturaResumen) => {
    ...
    if (factura.estado.idEstado === ESTADOS_FACTURA.PAGADO) {
        return {
            texto: formatEstadoNombre(factura.estado.nombre),
...
export default function FacturasPage() {
    ...
    // ==================== MANEJO DE IMPRESIÓN ====================
    const handleImprimirFactura = async (id: number) => {
        try {
    ...
    // Función para imprimir directamente desde la tabla (sin modal previo) con indicador de carga
    const handleImprimirRapido = async (id: number) => {
        ...
        try {
            ...
            // Cargar factura en paralelo
            const factura = await fetchFacturaDetalles(id);
            if (factura) {
                setFacturaParaImprimir(factura);
            } else {
                setShowPrintModal(false);  // Cerrar si falla
            }
        } catch (error) {
            console.error('Error:', error);
            toast.error('Error al cargar factura para impresión');
    ...
    const handleRegistrarPago = async () => {
        ...
        // Si es pago en efectivo, mostrar modal de cambio
    ...
    const procesarPago = async (idFactura: number, monto: number) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (updatedFactura) {
                    // Si el pago completó la factura, preguntar si desea imprimir
                    if ((updatedFactura.montoPendiente || 0) <= 0) {
                        setFacturaCreadaRecientemente(updatedFactura);
                        setShowPrintPromptModal(true);
                    }

                    // Si estamos en el modal de detalles, actualizarlo
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="bg-gradient-to-r from-orange-50 to-yellow-50 rounded-xl shadow-lg p-4 border border-orange-200">
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-sm text-orange-600">
                            {proximasEntregas.length} facturas
                        </span>
                    </div>
                    <div className="overflow-x-auto">
                        <div className="flex gap-3 pb-2">
                            {proximasEntregas.slice(0, 5).map((entrega) => (
                                <div
                                    key={entrega.idFactura}
                                    className="bg-white p-3 rounded-lg border border-orange-200 min-w-[250px] flex-shrink-0"
                                >
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-cyan-100">
                <div className="overflow-x-auto">
                    <table className="min-w-full">
                        ...
                        <tr>
                            <th className="px-6 py-3 text-left text-xs font-medium text-cyan-700 uppercase tracking-wider">
                                Número
                            </th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-cyan-700 uppercase tracking-wider">
                                Cliente
                                ...
                                <td colSpan={8} className="px-6 py-8 text-center text-gray-500">
                                    <div className="flex flex-col items-center">
                                        ...
                                        <p className="text-sm text-gray-400 mt-1">
                                            {filtros.search || filtros.estadoId || filtros.fechaDesde || filtros.fechaHasta
            ...
            {/* COMPONENTES MODALES - SOLO PASAR PROPS NECESARIAS */}
            <FacturasComponents
                showModal={showModal}
                ...
                // Funciones específicas que necesita el componente
                ...
                // Para manejar la impresión desde el detalle
            ...
            {/* MODAL PREGUNTAR SI IMPRIMIR FACTURA CREADA */}
                    ...
                    <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl border border-cyan-100">
                        <div className="text-center mb-4">
                            ...
                            <p className="text-gray-600">
                                La factura <span className="font-semibold text-cyan-600">{facturaCreadaRecientemente.numeroFactura}</span> ha sido {(facturaCreadaRecientemente.montoPendiente || 0) <= 0 ? 'pagada completamente' : 'creada'}.
                            </p>
                        </div>
                                ...
                                <div className="text-sm space-y-1">
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-bold text-cyan-600">
                                            {formatCurrency(parseFloat(pagoForm.monto) || 0)}
                                        </span>
                                    </div>
                                    <div className="flex justify-between">
